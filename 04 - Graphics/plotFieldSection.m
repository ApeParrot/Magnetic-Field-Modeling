function [] = plotFieldSection(R,L,Mvec)
...
[xPl,zPl] = meshgrid(linspace(-2*D,2*D,Npts),              ...
...
inside = ( abs(xPl) < R-delta ) & ( abs(zPl) < L-delta );
% outside = ~inside;
...
hold on
% contourf(xx,zz,log10(Hnorm),60,'edgecolor','none')
contourf(xx,zz,Hnorm,60,'edgecolor','none')
l = streamslice(xx,zz,[],H_x,H_z,[],[],[],6,.5);
set(l,'LineWidth',1.2)
set(l,'Color','w')
...
% view(0,-90)
contourf(xx,zz,Bnorm,60,'edgecolor','none')
l = streamslice(xx,zz,[],B_x,B_z,[],[],[],6,.5);
set(l,'LineWidth',1.2)
set(l,'Color','w')
rectangle('Position',[-R -L 2*R 2*L],'LineWidth',2)
hold off

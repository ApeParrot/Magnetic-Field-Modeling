%% Function drawRingMagnet
 % -----------------------------------------------------------------------
 % This function draws a ring magnet of length/height: L, internal radius
 % rin and external radius rout. The drawn magnet is translated according 
 % to the first three coordinates of magnetPose (cartesian coordinates) 
 % and oriented according to the last three coordinates of magnetPose 
 % (magnetic moment vector).
 %
 % Inputs: 
 %       1) L    : length/height of the magnet;                   [number]
 %       2) rin  : internal radius of the magnet;                 [number]
 %       3) rout : external radius of the magnet;                 [number]
...
function [] = drawRingMagnet(L,rin,rout,magnetPose,varargin)

if rin > rout
    error('rout value should be larger than rin!')
end
...
if strcmp(texture,'axial')
    ...
    [x1,y1,z1] = cylinder([rin,rout,rout,rin],300);
    z1 = z1*h/2;
    z1([1,2],:) = 0;
    z1([3,4],:) = h/2;
    z1 = z1-h/2;
    
    [x2,y2,z2] = cylinder([rin,rout,rout,rin],300);
    z2 = z2*h/2;
    z2([1,2],:) = 0;
    z2([3,4],:) = h/2;
...
%     shading flat
    ...
    [x,y,z] = cylinder([rin,rout,rout,rin],300);
    z = z*h;
    z([1,2],:) = 0;
    z([3,4],:) = h;
    z = z-h/2;
